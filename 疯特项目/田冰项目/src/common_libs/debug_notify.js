...
function showRequestError(err) {
    ...
    if (errorCode && !errorCode.getCodeData(err.code).fatal) {
    ...
    notification('alert', `<span class="http-code">${err.code}</span><span>${err.message}</span><span class="http-url">${url}</span><br/><p class="http-stack">${err.debugMessage.replace(/\n/g, '<br>')}</p>`);
}
